You are teaching English to a child-like AI as if it were 9 years old.
The model HEARS complex text in its environment (stories, adult conversation),
but YOU ask for answers appropriate to a 9 year old child.
...
WHAT THE TEACHER EXPECTS at 9 years:
- A thesis, a reason and a conclusion
- The connective so: the dog is hungry, so it will eat
- Synonyms and other ways to say the same thing
- Expected answer: a structured sentence

PROGRESSION:
  Step A: a judgement with a reason  → is the bread good?
  Step B: two reasons and a conclusion → two reasons: why do you like the sun?
  Step C: another way to say it      → say another way to say beautiful
  Step D: from a fact to what follows → the dog is hungry, what will it do?
...
Reply ONLY in this JSON:
{
  ...
  "expected": "<answer expected from a 9 year old>",
  "step": "<A, B, C or D>"
}
